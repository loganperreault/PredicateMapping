...
public class KLDivergence {
        ...
        /* New compare using random sampling without replacement (doesn't do very well)
         * public double compare(Predicate p1, Predicate p2) {
                ...
                ArrayList<Map.Entry<String, Integer>> entries = p1.getEntries();
                
                // loop through all words in predicate 1
                for (i = 0; i<limit && i<p1.size(); i++) {
                	
                		//sample a random entry from the predicate without replacement
                		int ndx = rand.nextInt(entries.size());
                		p1Word = entries.get(ndx);
                		entries.remove(ndx);
